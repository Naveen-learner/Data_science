# -*- coding: utf-8 -*-
"""
Created on Wed Oct 12 14:23:25 2022
...
transact_list = []
for i in res:
    transact_list.append(i.split(","))

type(transact_list1[0][5])
    
transact_list1 = [[ subelt for subelt in elt if subelt != "NA" ] for elt in transact_list]

        
### Elemantary Analysis ###
all_transact_list = [i for item in transact_list1 for i in item] #extracting all items

all_transact_list = list(filter(None, all_transact_list))
...
plt.bar(height = frequencies[0:2620], x = list(range(0, 2620)), color = ['red','green','black','yellow','blue','pink','violet'])
plt.bar(height = frequencies[0:2620], x = list(range(0, 2620)), color = ['red','green','black','yellow','blue','pink','violet'])

plt.xticks(list(range(0, 2620), ), items[0:2620])
plt.xlabel("items")
plt.ylabel("Count")
plt.show()
##########
...
transact_series = pd.DataFrame(pd.Series(transact_list1))
transact_series = transact_series.iloc[:4226, :] # removing the last empty transaction
