...
Univ1 = pd.read_excel(r"C:\Users\Naveen Kumar\Desktop\Data Science\Class\Datasets\University_Clustering.xlsx")
...
Univ1 = Univ.iloc[:, [7,0,1,2,3,4,5,6]] #re-order columns
Univ1.head()

# Aggregate mean of each cluster for labellling the cluster
#only clustering is success when we are able label it
Univ1.iloc[:, 2:].groupby(Univ1.clust).mean() #groupping by mean clustering
Univ1.iloc[:, 2:].groupby(Univ1.clust).std() #groupping by std clustering
# creating a csv file 
